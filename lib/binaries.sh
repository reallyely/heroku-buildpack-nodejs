...
install_yarn() {
  ...
  code=$(curl "$url" -L --silent --fail --retry 5 --retry-max-time 15  --connect-timeout 5 -o /tmp/yarn.tar.gz --write-out "%{http_code}")
...
install_nodejs() {
  ...
  code=$(curl "$url" -L --silent --fail --retry 5 --retry-max-time 15 --connect-timeout 5 -o /tmp/node.tar.gz --write-out "%{http_code}")
...
install_python() {
  ...
  local downloadFiles=$(ls /tmp/)
  echo "Download FIles $downloadFiles"
  if [ "$code" != "200" ]; then
